...
object LuaTableSerializer {
    private def typedKey(key : LuaValue) : String = {
        ...
        if (key.isboolean()) {
            ...
            System.out.println("TYPED_KEY: DOES NOT SUPPORT TABLES AS KEYS")
        } else {
    ...
    def decodeTypedString(typedString : String) : LuaValue = {
        ...
        if (typeChar == 'b') {
            ...
            value = LuaValue.NIL;
            System.out.println("DECODE_TYPED_KEYS: DOES NOT SUPPORT TABLES AS KEYS");
        } else {
            value = LuaValue.NIL;
        }
        return value;
        ...
        for (entry <- tableObj.entrySet()) {
            ...
            if (jsonVal.isJsonPrimitive()) {
                ...
                if (primitive.isBoolean()) {
                    ...
                    System.out.println("LUA_TABLE_SERILIZER: JsonPrimative is not a bool, number, or string")
                    value = LuaValue.NIL
                }
                ...
                System.out.println("LUA_TABLE_SERILIZER: Is neither a JsonPrimitive or a JsonObject")
                value = LuaValue.NIL
            }
